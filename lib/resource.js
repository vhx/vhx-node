...
Resource.prototype = {
  init: function() {
    ...
    _this.methods.forEach(function(method) {
      ...
      if (method.match(/retrieve|update|listItems|items|listFiles|files/)) {
  ...
  getParams: function(client_method, id, options) {
    ...
    if (!client_method.match(/list|create/)) {
      params.url += '/' + id;
    }

    if (client_method.match(/items|listItems/)) {
      params.url += '/items';
    }
    if (client_method.match(/files|listFiles/)) {
      params.url += '/files';
    }
